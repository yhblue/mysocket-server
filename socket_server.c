...
struct write_buffer {
	...
	char *ptr;		// 指向当前块未发送缓冲区首字节 --就是当前buffer，一部分已经拷贝给内存缓冲区了
	                //另一部分还没有，指向还没有的那一部分的首地址
	int sz;			// 当前块未发送的字节数
	void *buffer;	// 发送缓冲区
};
...
open_socket(struct socket_server *ss, struct request_open * request, struct socket_message *result, bool blocking) {
	...
	if(status == 0) {	// 说明connect已连接成功
		...
		sp_write(ss->event_fd, ns->fd, ns, true);	// 非阻塞套接字尝试连接中，必需将关注其可写事件，
		                     					    //稍后epoll才能捕获到连接出错了还是成功了
	}
...
static int
start_socket(
	struct socket_server *ss, struct request_start *request, struct socket_message *result) {
	int id = request->id;
	result->id = id;
	result->opaque = request->opaque;
	result->ud = 0;
	...
	for (;;) 
	{
		...
		if (ss->checkctrl) 
		{
			if (has_cmd(ss)) 
			{
				...
				if (type != -1)
					return type; //直接返回了，不检测epoll
				else
					continue; //再次循环，还是会检测管道，如果管道还是有命令继续读，处理。
				              //直到管道空则上一次的64个epoll中的socket向管道发送的命令已经处理完成。
				              //那么就进行下一次的epoll_wait
			} else 
			{ // 没有控制命令
				ss->checkctrl = 0;
		...
		switch (s->type) //socket的类型，根据socket类型判断是哪一种类型的fd发生变化了
		{
